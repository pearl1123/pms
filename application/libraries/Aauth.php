...
class Aauth {
	...
	public function login($identifier, $pass, $remember = false, $totp_code = null) {
		...
		if ($this->config_vars['ddos_protection'] && ! $this->update_login_attempts($identifier)) {

			$this->error($this->CI->lang->line('aauth_error_login_attempts_exceeded'));
			return false;
		}
		if($this->config_vars['ddos_protection'] && $this->config_vars['recaptcha_active'] && $this->get_login_attempts($identifier) > $this->config_vars['recaptcha_login_attempts']){
        ...
        $query = $this->aauth_db->where('email_verify', 1);
	...
	public function loginFromApi($identifier, $pass, $remember = false, $totp_code = null) {
	 ...
	 */
	public function reset_login_attempts($identifier) {
	 ...
	 * @return bool
	 */
	public function update_login_attempts($identifier) {
		$ip_address = $identifier;
	 ...
	 * @return int
	 */
	public function get_login_attempts($identifier) {
		$ip_address = $identifier;
		$query = $this->aauth_db->where(
			array(
				'ip_address'=>$ip_address,
	 ...
	 */
public function create_user($email, $pass, $fullname, $phone_number, $office, $username = false)
{
    $valid = true;

    // --- Validation Checks ---
    if ($this->config_vars['login_with_name'] == true && empty($username)) {
    ...
    if (strlen($pass) < $this->config_vars['min'] || strlen($pass) > $this->config_vars['max']) {
    ...
    $data = [
        ...
        'fullname'      => $fullname, // ✅ ADD THIS - it's required in DB
        'phone_number'  => $phone_number,
        'office'        => $office,
    ];

    $db = isset($this->aauth_db) ? $this->aauth_db : $this->db;

    // Debug: Log the data being inserted
    log_message('debug', 'Attempting to insert user data: ' . print_r($data, true));

    if ($db->insert($this->config_vars['users'], $data)) {
        $user_id = $db->insert_id();
        log_message('debug', 'User inserted successfully with ID: ' . $user_id);
        ...
        if ($this->config_vars['verification'] && !$this->is_admin()) {
            $db->where('id', $user_id);
            $db->update($this->config_vars['users'], ['banned' => 1]);
            $this->send_verification($user_id);
        }

        // Update password with salted hash (if needed)
        if (!$this->config_vars['use_password_hash']) {
            $db->where('id', $user_id);
            $db->update($this->config_vars['users'], [
                'pass' => $this->hash_password($pass)
            ]);
        }

        // ✅ Fetch the created user with ward info
        // $query = $db->select('u.*, w.ward_name')
        //     ->from($this->config_vars['users'] . ' u')
        //     ->join('lib_wards w', 'w.ward_id = u.ward_id', 'left')
        //     ->where('u.id', $user_id)
        //     ->get();

        // Check if query was successful before calling row_array()
        if ($query) {
            return $query->row_array();
        } else {
            log_message('error', 'Failed to fetch created user data');
            return false;
        }
        ...
        $this->error('Database insert failed: ' . $error['message']);
        return false;
    }
}
	//tested
	...
	public function update_user(
		...
		if ($pass !== false) {
			if (strlen($pass) < $this->config_vars['min'] || strlen($pass) > $this->config_vars['max']) {
				...
				$data['pass'] = $this->hash_password($pass);
			}
		}

		// --- USERNAME ---
		if ($username !== false && $user->username != $username) {
			if ($this->user_exist_by_username($username)) {
				$this->error($this->CI->lang->line('aauth_error_update_username_exists'));
				$valid = false;
			} elseif ($username != '' && !ctype_alnum(str_replace($this->config_vars['additional_valid_chars'], '', $username))) {
				$this->error($this->CI->lang->line('aauth_error_username_invalid'));
				$valid = false;
			} else {
	 ...
	 */
	function hash_password2($pass, $userid) {
		if($this->config_vars['use_password_hash']){
	 ...
	 */
	function verify_password2($password, $hash) {
		if($this->config_vars['use_password_hash']){
	...
	public function error($message = '', $flashdata = true){
		...
		if($flashdata){
			...
			$this->CI->session->set_flashdata('fail', $message);
		}
	}
