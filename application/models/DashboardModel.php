...
<?php
    ...
    class DashboardModel extends CI_Model
    {
        ...
        public function getPPMPForReviewListView($start, $length)
        {
            $this->db->select('DISTINCT(t1.ppmp_year), t1.ppmp_id');
            $this->db->where(array('t1.archived' => 0));
            $this->db->where('t2.current_status', 'FOR_REVIEW');
            $this->db->where('t2.current_holder', $this->session->userdata('userID'));
            $this->db->join('tbl_ppmp_current_status t2', 't2.ppmp_id = t1.ppmp_id AND t2.archived = 0', 'left');
